#INPUT DATA
  recombinationSitesHeader = #Gaculeatus_linkagemap_header.txt
  recombinationSitesThreespine = #Gaculeatus_linkagemap.txt

  referenceFai = #Index (.fai) of the three-spined stickleback reference genome (Peichel et al. 2017)
...
# STEP 0: PRE-PROCESSSING IN BASH
      #    tr -d '*' < recombinationSitesThreespine| cut --complement -f158,179,255,272,275,277,290,408,417,432,476 > out1  ## trim asterisks and remove duplicate offsprings with cut
      #    cut --complement -f158,179,255,272,275,277,290,408,417,432,476 recombinationSitesHeader > out2

#

#STEP 1: Concatenate the recombinations
      header = read.table(out2,sep="\t",stringsAsFactors=F)#From the STEP 0
      IN = read.table(out1,sep="\t",stringsAsFactors=F)#From the STEP 0
...
#### STEP 2: Run the EM algorithm
		    ...
		    lgs = paste0("chr",as.roman(seq(1,21)))
		    lgs=lgs[1:21]
		    ...
		    for(LG in lgs){
	 		...
	 		OUT[["PATERNAL"]]=OUT.PATERNAL
	 		OUT[["MATERNAL"]]=OUT.MATERNAL
...
# STEP 3 Predict the recombination frequencies
        ...
        table1=table.out #Step 1
        table2=metadata
        IN= OUT #From step 2
	    table.out=data.frame()
	    library(ggplot2)
		####
	    source(p0kFunction) #activate the p_0(k) function
	    ...
	    rbar.out=data.frame()
	for(lg in 1:21){
			t = read.table(paste0("chr", lg, "_threeSpine.rf.gz"),header=F)#Empirical recombination frequencies and map distances based on linkage maps
			t=subset(t,V1>V2)
	     ...
	     ##   t=t[sort(sample(1:nrow(t),5000,replace=F)),] #for quicker execution
			...
			t$recomb=t$V3/100
			t$CHR=paste0("chr",as.roman(lg))
			t$CONTEXT=paste(ceiling((t$V1+0.00001)/25),ceiling((t$V2+0.00001)/25),sep="-")
			both=rbind(both,t)
			...
			labs(fill="Empirical data", colour="Mapping function",title=paste0("chr",as.roman(lg)))  +    
			...
			ggsave(paste0("chr",lg,"empirical.png"),device="png",width=100,height=60,units="mm",dpi=100)
			
			p=p+facet_wrap(~V4,ncol=2)
            ggsave(paste0("chr",lg,"empirical_2panels.png"),device="png",width=100,height=60,units="mm",dpi=100)
			...
			scale_color_manual(values=c("#F0E442","#009E73","orchid","tan3","firebrick")) + 
			scale_fill_manual(values=c("tan1","steelblue1")) + 
			labs(fill="Empirical data", colour="Mapping function"	)  +    
			guides(fill = guide_legend(override.aes = list(alpha=1,size=2))) +
			theme(legend.position="right",aspect.ratio=1)#+facet_wrap(~CONTEXT,ncol=6)
			ggsave(paste0("chr",lg,"empirical_female.png"),device="png",width=100,height=60,units="mm",dpi=300)
			...
			guides(fill = guide_legend(override.aes = list(alpha=1,size=2))) +
			theme(legend.position="right",aspect.ratio=1)#+facet_wrap(~CONTEXT,ncol=6)
			ggsave(paste0("chr",lg,"empirical_male.png"),device="png",width=100,height=60,units="mm",dpi=300)
           ...
           rbar.tmp=data.frame(Chr=lg,Sex=c("Maternal","Paternal"), Nrbarloci=c(rbar.loci.female,rbar.loci.male), SumRij=as.numeric(aggregate(recomb ~ V4, sum, data=plot.table1.rbar)$recomb),Source="Empirical")
		    ...
		    plot.p0k=data.frame()
		       ...
		       for(sex in c("Paternal","Maternal")){
			       n=max(which(metadata[metadata$CHR==paste0("chr",as.roman(lg)),paste0(casefold(sex,upper=T),"MLEp",1:9)]>0))
			       pk=as.numeric(metadata[metadata$CHR==paste0("chr",as.roman(lg)),paste0(casefold(sex,upper=T),"MLEp",1:9)])
			       p_0=as.numeric(metadata[metadata$CHR==paste0("chr",as.roman(lg)),paste0(casefold(sex,upper=T),"MLEp",0)])
			       tmp.meta=IN[[casefold(sex,upper=T)]]
			       if(as.numeric(tmp.meta[tmp.meta$CHR==paste0("chr",as.roman(lg)),"BootstrapPvalue"])>=0.05){
			               pk=as.numeric(tmp.meta[tmp.meta$CHR==paste0("chr",as.roman(lg)),grepl("MLEpRestricted",colnames(tmp.meta))])
			               p_0=pk[1]
			               pk=pk[-1]
				   }
			       tmp=subset(t,V4==sex)
			       length.cm=max(tmp$V1,tmp$V2,as.numeric(metadata[metadata$CHR==paste0("chr",as.roman(lg)),paste0("CHRLENGTHCM",casefold(sex,upper=T))]))
			...
			scale_color_manual(values=c("#F0E442","#009E73","orchid","tan3","firebrick")) + 
			scale_fill_manual(values=c("tan1","steelblue1")) + 
			labs(fill="Prediction", colour="Mapping function",title=paste0("chr",as.roman(lg)))  +    
			guides(fill = guide_legend(override.aes = list(alpha=1,size=2))) +
			theme(legend.position="right",aspect.ratio=1)#+facet_wrap(~CONTEXT,ncol=6)
			ggsave(paste0("chr",lg,"prediction.png"),device="png",width=100,height=60,units="mm",dpi=100)
			
			p=p+facet_wrap(~SEX,ncol=2)
            ggsave(paste0("chr",lg,"prediction_2panels.png"),device="png",width=100,height=60,units="mm",dpi=100)
			...
			guides(fill = guide_legend(override.aes = list(alpha=1,size=2))) +
			theme(legend.position="right",aspect.ratio=1)#+facet_wrap(~CONTEXT,ncol=6)
			ggsave(paste0("chr",lg,"prediction_female.png"),device="png",width=100,height=60,units="mm",dpi=300)
			...
			guides(fill = guide_legend(override.aes = list(alpha=1,size=2))) +
			theme(legend.position="right",aspect.ratio=1)#+facet_wrap(~CONTEXT,ncol=6)
			ggsave(paste0("chr",lg,"prediction_male.png"),device="png",width=100,height=60,units="mm",dpi=300)
		    ...
		    xlim(0,160)+ xlab("Map distance (cM)")+ylab("Error")+
		    scale_fill_manual(values=c("tan1","steelblue1")) + 	
		    labs(fill="Sex",title=paste0("chr",as.roman(lg)))  +    
		    guides(fill = guide_legend(override.aes = list(alpha=1,size=2))) +
		    theme(legend.position="none",text=element_text(size=5))+
		    facet_grid(V4~mappingFunction,scales="free_y")
		    ggsave(paste0("chr",lg,"residual_scatter.png"),device="png",width=100,height=60,units="mm",dpi=250)
		    ...
		    tmp.sum.of.squares$likelihood=aggregate(t2$likelihood,by=list(t2$V4,t2$mappingFunction,t2$CHR),sum)$x
	...
	tmp.sum.of.squares$TotalAbsoluteError=aggregate(error.absolute ~ V4 + mappingFunction + CHR,sum,data=t2)$error.absolute		   
...
### STEP 4 Collect the results
		...
		sum.of.squares.total=aggregate(sq.sum ~ V4 + mappingFunction, sum, data=sum.of.squares)
                sum.of.squares$chromosome=factor(sum.of.squares$chromosome,levels=paste0("chr",as.roman(1:21)))
                likelihood.total= aggregate(logLikelihood ~ V4 + mappingFunction, sum, data=sum.of.squares)
 
		 best=aggregate(logLikelihood ~ chromosome+V4,data=sum.of.squares,max)
		 best=subset(sum.of.squares,logLikelihood %in% best$logLikelihood)# & mappingFunction %in% c("Our","Our_un"))
		 best=subset(best, !(duplicated(logLikelihood)))
		 best$LABEL="*"
		 ...
		 sum.of.squares$mappingFunction=factor(sum.of.squares$mappingFunction,labels=c("Haldane", "Kosambi", "Linear", "p0(k)"))
		 ggplot(sum.of.squares,aes(x=chromosome,y=logLikelihood,fill=mappingFunction))+geom_jitter(shape=23,stroke=0.1,alpha=0.75,size=1,width=0.05,height = 0)+#geom_point(shape=23,stroke=0.1,alpha=0.8,size=0.5)+
		 ...
		 ggsave("SumOfLikelihoodsOfData.pdf",device="pdf",width=160,height=60,units="mm")
		  ...
		  plot.table.error$MeanAbsoluteError=aggregate(TotalAbsoluteError ~ V4 + mappingFunction, sum,data=table1)$TotalAbsoluteError/plot.table.error$NumberOfMarkers
		  plot.table.error$TotalErrorSd=aggregate(TotalErrorSd ~ V4 + mappingFunction, mean, data=table1)$TotalErrorSd
		  
		  
		  ggplot(subset(plot.table.error, mappingFunction!="p0(k)*"),aes(x=mappingFunction,y=MeanError,ymin=MeanError-TotalErrorSd,ymax=MeanError+TotalErrorSd,fill=mappingFunction))+
		  geom_hline(yintercept=0,linetype=2)+
		  ...
		  theme(legend.position="bottom",text=element_text(size=6),panel.grid=element_blank(),panel.background=element_blank(), axis.line = element_line(colour = "black"), panel.border = element_rect(fill=NA))
		  ggsave("TotalError.png",device="png",width=110,height=80,units="mm",dpi=1000)
		 
		 
		 
		 OUT[["SumOfSquares"]]=sum.of.squares.total
		 OUT[["logLikelihoods"]]=likelihood.total
...
#STEP 5: Calculate rbar according to (Veller et al. 2019)
		...
		print(rbar)
	    rbar=subset(rbar, Chr !=19)
	    rbar$IntraContrib=rbar$SumRij/rbar$Nrbarloci
	    
	    rbar$ChrLength=rep(table1$LENGTH[-19], each=12)
	    
	    rbar$IntraContrib2=rbar$IntraContrib*((rbar$ChrLength/sum(as.numeric(table1$LENGTH[-19])))^2)
	    
	    print(aggregate(IntraContrib ~ Sex + Source, sum,data=rbar))
	    print(aggregate(IntraContrib2 ~ Sex + Source, sum,data=rbar))
	    
	    inter.chrom=data.frame(Species="Gacu",InterChromComponent=0.5*(1-sum((table1$LENGTH[-19]/sum(table1$LENGTH[-19]))^2)))
